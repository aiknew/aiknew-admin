...
const isGroup = (route: RouteRecordRaw) => route?.meta?.type === 'GROUP' && !route?.meta?.hidden
const isMenuItem = (route: RouteRecordRaw) =>
  route?.meta?.type === 'MENU' && !hasChildren(route.children) && !route?.meta?.hidden
...
<template>
  <template v-for="route in routes" :key="route.path">
    ...
    <template v-if="route.children && route.children.length && isGroup(route)">
      <el-sub-menu :index="route.path" :key="route.path">
        <template #title>
          ...
          <span class="menu-title">{{ getRouteName(route.meta?.translations) }}</span>
        </template>
    ...
    <!-- normal menu item -->
    <el-menu-item v-if="isMenuItem(route)" :index="route.path" :key="route.name">
      <el-icon v-if="route.meta?.icon">
        <component :is="route.meta.icon" />
      </el-icon>
      <span class="menu-title">{{ getRouteName(route.meta?.translations) }}</span>
