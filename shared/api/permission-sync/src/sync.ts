...
import {
  ...
  IS_PUBLIC_KEY,
  PERMISSION_GROUP_DECORATOR_KEY,
  PERMISSION_KEY,
...
export const syncPermission = async ({
  ...
  const permissionsData: (Record<string, unknown> & {
  ...
  try {
    for (const controller of controllers) {
      const controllerPath = resolve(
        adminBasePath,
        reflector.get<string>(PATH_METADATA, controller.instance.constructor),
      )
      const controllerHandlers = metadataScanner.getAllMethodNames(
        controller.instance,
      )
      const permissionGroupMeta = reflector.get<
        PermissionGroupMetaData | undefined
      >(PERMISSION_GROUP_DECORATOR_KEY, controller.instance.constructor)
      ...
      if (permissionGroupMeta) {
        groupTranslations = langKeys.map((langKey) => {
          const groupName: string = get(
            allTranslations[langKey],
            permissionGroupMeta.name,
          )
      ...
      for (const handlerName of controllerHandlers) {
        ...
        // is authenticated
        const isAuthenticated = reflector.getAllAndOverride<boolean>(
          IS_AUTHENTICATED_KEY,
        ...
        const reqMethod = RequestMethod[
          reflector.get(METHOD_METADATA, handler)
        ] as keyof typeof RequestMethod
        const fullPath = resolve(controllerPath, handlerPath)
        const handlerMeta = reflector.get<PermissionMetaData | undefined>(
          PERMISSION_KEY,
          handler,
        )
        ...
        if (handlerMeta) {
          const translations = langKeys.map((langKey) => {
            const permissionName: string = get(
              allTranslations[langKey],
              handlerMeta.name,
            )
          ...
          // warning
          console.warn(
            `${controller.name}:${handlerName}didn't have @Public decorator or @Permission decorator or @Authenticated on it,it can not be access by default`,
          )
        }
      }
    }
    ...
    await writeToTsFile(
      "../db/admin-db/prisma/seeds/admin-data/admin-permissions/permissions.ts",
      permissionsData,
    )
  } catch (err) {
    console.log("err: ", err)
