...
async function main() {
  ...
  try {
    await syncPermission({ discovery, i18n, metadataScanner, prisma, reflector, adminBasePath })
  } catch (err) {
    console.log('err: ', err)
  } finally {
    await app.close()
